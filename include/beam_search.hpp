...
class BeamSearch {
  public:
     ...
     * @param config Reference to transformer configuration
     ...
     */
    BeamSearch(const TransformerConfig& config,
  ...
  private:
    ...
    size_t top_k;
    float top_p;
    size_t max_length_;  // Maximum number of tokens to generate
    const TransformerConfig& config_;  ///< Reference to transformer configuration
    ...
    std::vector<Hypothesis> get_best_sequence(
    ...
    void process_projected_scores(std::vector<std::vector<int>>& sequences,
                                Matrix& beam_scores,
